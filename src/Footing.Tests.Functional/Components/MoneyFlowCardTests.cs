...
public class MoneyFlowCardTests : BunitContext
{
    ...
    [Fact]
    public void MoveOnButton_NotShown_WhenNoAdvanceCallback()
    {
        var cut = RenderCard();
        cut.FindAll(".ft-move-on__btn").Count.Should().Be(0);
    }

    [Fact]
    public void MoveOnButton_Shown_WhenAdvanceCallbackProvided()
    {
        var flows = new MoneyFlows { Direction = MoneyFlowDirection.Income };
        var cut = Render<MoneyFlowCard>(p => p
            .Add(c => c.MoneyFlows, flows)
            .Add(c => c.Name, "test")
            .Add(c => c.Label, "Test")
            .Add(c => c.FormPrompt, "Enter")
            .Add(c => c.FormAmountDescriptionConnectorWord, "for")
            .Add(c => c.IsOpen, true)
            .Add(c => c.OnAdvanceRequested, () => { }));

        cut.FindAll(".ft-move-on__btn").Count.Should().Be(1);
    }

    [Fact]
    public void MoveOnButton_GetsReadyClass_AfterFirstEntry()
    {
        var flows = new MoneyFlows { Direction = MoneyFlowDirection.Income };
        var cut = Render<MoneyFlowCard>(p => p
            .Add(c => c.MoneyFlows, flows)
            .Add(c => c.MoneyFlowsChanged, _ => { })
            .Add(c => c.Name, "test")
            .Add(c => c.Label, "Test")
            .Add(c => c.FormPrompt, "Enter")
            .Add(c => c.FormAmountDescriptionConnectorWord, "for")
            .Add(c => c.IsOpen, true)
            .Add(c => c.OnFirstEntryAdded, () => { })
            .Add(c => c.OnAdvanceRequested, () => { }));

        cut.Find(".ft-move-on__btn").ClassList.Should().NotContain("ft-move-on__btn--ready");

        cut.Find("input[placeholder='xxx.xx']").Change("500");
        cut.Find("select").Change("Weekly");
        cut.Find("input[placeholder='Test Description']").Change("Salary");
        cut.Find("button[type='submit']").Click();

        cut.Find(".ft-move-on__btn").ClassList.Should().Contain("ft-move-on__btn--ready");
    }

    [Fact]
    public void MoveOnButton_InvokesAdvanceCallback()
    {
        ...
        var cut = Render<MoneyFlowCard>(p => p
            ...
            .Add(c => c.FormAmountDescriptionConnectorWord, "for")
            .Add(c => c.IsOpen, true)
            .Add(c => c.OnAdvanceRequested, () => { advanceCalled = true; }));

        cut.Find(".ft-move-on__btn").Click();

        advanceCalled.Should().BeTrue();
    }

    [Fact]
    public void FirstEntry_InvokesFirstEntryCallback_NotAdvance()
    {
        var firstEntryCalled = false;
        var advanceCalled = false;
        var flows = new MoneyFlows { Direction = MoneyFlowDirection.Income };
        var cut = Render<MoneyFlowCard>(p => p
            .Add(c => c.MoneyFlows, flows)
            ...
            .Add(c => c.FormAmountDescriptionConnectorWord, "for")
            .Add(c => c.IsOpen, true)
            .Add(c => c.OnFirstEntryAdded, () => { firstEntryCalled = true; })
            .Add(c => c.OnAdvanceRequested, () => { advanceCalled = true; }));
        ...
        firstEntryCalled.Should().BeTrue();
        advanceCalled.Should().BeFalse();
    }
}
